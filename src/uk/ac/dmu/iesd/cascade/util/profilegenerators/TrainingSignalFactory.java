...
/**
 * Factory to create training signals of a requested shape. The shape is offset
 * by one tick each day
 * 
 * Supported shapes are enumerated in
 * {@link TrainingSignalFactory.TRAINING_S_SHAPE}
 * 
 * 
 * @author jsnape
 * 
...
public class TrainingSignalFactory
{
	...
	private double[] PBsigFromMatlab = new double[]
	{ 0, 0.166666667, 0.25, 0.333333333, 0.416666667, 0.5, 0.583333333, 0.666666667, 0.75, 0.833333333, 0.916666667, 1, 1, 0.916666667,
			0.833333333, 0.75, 0.666666667, 0.583333333, 0.5, 0.416666667, 0.333333333, 0.25, 0.166666667, 0, 0, -0.166666667, -0.25,
			-0.333333333, -0.416666667, -0.5, -0.583333333, -0.666666667, -0.75, -0.833333333, -0.916666667, -1, -1, -0.916666667,
			-0.833333333, -0.75, -0.666666667, -0.583333333, -0.5, -0.416666667, -0.333333333, -0.25, -0.166666667, 0 };

	private int signalLength;

	/**
	 * Training signal shapes available, currently : {IMPULSE, TRIANGLE, SQUARE,
	 * SINE, PBORIGINAL, COSINE}
	 */
	public static enum TRAINING_S_SHAPE
	{
		IMPULSE, TRIANGLE, SQUARE, SINE, PBORIGINAL, COSINE
	};
	...
	{
		return this.generateSignal(sigType, this.signalLength);
	}
	...
	public double[] generateSignal(TRAINING_S_SHAPE sigType, int n)
	{
		...
		{
		case IMPULSE:
			ret = this.genImpulseSignal(n);
			break;
		case TRIANGLE:
			ret = this.genTriangularSignal(n);
			break;
		case SQUARE:
			ret = this.genSquareSignal(n);
			break;
		case SINE:
			ret = this.genSineSignal(n);
			break;
		case COSINE:
			ret = this.genCosineSignal(n);
			break;
		case PBORIGINAL:
	...
	/**
	 * Note that slightly awkward way of doing this is to get a true zero at the
	 * mid point rather than a "nearly zero" due to double precision.
	 * 
	 * @param n
	 * @return
	 */
