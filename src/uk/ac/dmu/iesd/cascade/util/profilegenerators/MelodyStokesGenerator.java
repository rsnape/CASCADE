...
public class MelodyStokesGenerator extends BasicProfileGenerator
{
	...
	public static WeakHashMap<String, double[]> melodyStokesColdApplianceGen(int numDays, boolean fridges, boolean fridgeFreezers,
			boolean freezers)
	{
		return MelodyStokesGenerator.melodyStokesColdApplianceGen(numDays, fridges ? 1 : 0, fridgeFreezers ? 1 : 0, freezers ? 1 : 0);
	}

	/**
	 * Helper function to generate only cold appliance profiles for backward
	 * compatibility
	 */
	public static WeakHashMap<String, double[]> melodyStokesColdApplianceGen(int numDays, int fridges, int fridgeFreezers, int freezers)
	{
		...
		Date[] startAndEnd = MelodyStokesGenerator.dates_1();
		Date start = startAndEnd[0];
		Calendar testCal = new GregorianCalendar();
		testCal.setTime(start);
		// Note - store the (int) day of year as a double as it is used
		// throughout
		...
		MelodyStokesGenerator.cooling(coldOnlyProfile, startDayofYear, startDayofWeek, numDays);
		MelodyStokesGenerator.HH_specific(coldHouseChars, false, coldOnlyProfile, start, numDays);
		...
		// Convert kW to kWh
		d_fridge = MelodyStokesGenerator.convertToKWh(d_fridge, true);
		d_freezer = MelodyStokesGenerator.convertToKWh(d_freezer, true);
		d_fridge_freezer = MelodyStokesGenerator.convertToKWh(d_fridge_freezer, true);
	 ...
	 * appliance demand (washing.m). Note that this implementation does not
	 * account for leap years and simply assumes that days per year or where in
	 * the year the simulation starts In effect, it assumes a 1st January start.
	 * 
	 ...
	 */
	public static double[] melodyStokesWetApplianceGen(int numDays, boolean washMachine, boolean washerDryer, boolean dishWasher,
			boolean tumbleDryer)
	{
		// TODO Auto-generated method stub
		return MelodyStokesGenerator
				.melodyStokesWetApplianceGenWithWeekends(numDays, washMachine ? 1 : 0, washerDryer ? 1 : 0, dishWasher ? 1 : 0, tumbleDryer ? 1
						: 0);
	}
	 ...
	 * account for leap years and simply assumes that days per year or where in
	 * the year the simulation starts In effect, it assumes a 1st January start.
	 * * Note - this model does not take account of different values for Economy
	 * 7 (E7) tariff as this is the effect we hope to model in our wider
	 * experiment. Hence, unrestricted (UR) values are used for the inital
	 * profile generation.
	 * 
	 * TODO: If this is to be used extensively, need to make it sensitive to
	 * start date etc.
	 ...
	 *         these may require conversion to kWh.
	 */
	private static double[] melodyStokesWetApplianceGenWithWeekends(int numDays, int washMach, int washDry, int dishWash, int tumbleDry)
	{
		...
		MelodyStokesGenerator.washing(wetOnlyProfile, startDayofYear, startDayofWeek, numDays);
		MelodyStokesGenerator.HH_specific(wetHouseChars, false, wetOnlyProfile, start, numDays);
		...
		// Convert kW to kWh
		d_washer_UR = MelodyStokesGenerator.convertToKWh(d_washer_UR, true);
		d_dryer_UR = MelodyStokesGenerator.convertToKWh(d_dryer_UR, true);
		d_dish_UR = MelodyStokesGenerator.convertToKWh(d_dish_UR, true);
	 ...
	 * the year the simulation starts In effect, it assumes a 1st January start.
	 * 
	 * Note - this model does not take account of different values for Economy 7
	 * (E7) tariff as this is the effect we hope to model in our wider
	 * experiment. Hence, unrestricted (UR) values are used for the inital
	 * profile generation.
	 * 
	 * TODO: If this is to be used extensively, need to make it sensitive to
	 ...
	 *         these may require conversion to kWh.
	 */
	private static WeakHashMap<String, double[]> melodyStokesWetApplianceGenWithDutyCycles(CascadeContext context, int numDays,
	...
	/**
	 * Translation of the main Melody file (LoadModel.m). This could be equated
	 * to a "main" type method for this class.
	 * 
	 * @param batch
	 * @param domestic
	 ...
	 * @param solarPV
	 */
	private static void generate(boolean batch, boolean domestic, boolean occupancyProfiling, int commercialBuildingType,
			boolean solarThermal, boolean solarPV, boolean oneMinRequired)
	{
		int consumer_type = 0;
		...
		if (!batch)
		{
			if (domestic)
			{
				...
				// Note - store the (int) day of year as a double as it is used
				// throughout
				// the original code in a division which returns a fractional
				// result -
				// Matlab is int / double agnostic.
				double startDayofYear = testCal.get(Calendar.DAY_OF_YEAR);

				// Note - Java returns days of week as 1-7, Sunday == 1,
				// Saturday == 7
				int startDayofWeek = testCal.get(Calendar.DAY_OF_WEEK);

				// Day of the year on which we change BST to GMT and vice versa.
				// This is currently fixed on a rough average - could be done
				// better from
				...
				MelodyStokesGenerator
						.HH_group(consumer_type, thisHHProfile, startDayofYear, startDayofWeek, BST_to_GMT_day, GMT_to_BST_day, Nspan);
				MelodyStokesGenerator.HH_specific(thisHouseChars, false, thisHHProfile, MelodyStokesGenerator.dates_1()[0], Nspan);

				double[] one_min_wash_profile;

				if (!occupancyProfiling)
				{
					one_min_wash_profile = MelodyStokesGenerator.one_min(thisHouseChars, thisHHProfile, start, Nspan);
				}
				else
				{
					one_min_wash_profile = MelodyStokesGenerator
							.one_min_profile(thisHHProfile, startDayofWeek, 16, 36, thisHouseChars, start);
				}

				double[] HH_wash_with_duty_cycle = MelodyStokesGenerator.aggregate_one_min_kWh_to_half_hour_kWh(MelodyStokesGenerator
						.convertToKWh(one_min_wash_profile, false));
			}
			else
			{
				consumer_type = 2;
				...
				{
				case 1:
					MelodyStokesGenerator.shops(consumer_type);
					break;
				case 2:
					MelodyStokesGenerator.offices(consumer_type);
					break;
				case 3:
					MelodyStokesGenerator.schools(consumer_type);
					break;
				case 4:
					MelodyStokesGenerator.hotels(consumer_type);
					break;
				case 5:
					MelodyStokesGenerator.church(consumer_type);
					break;
				default:
		...
		{
			MelodyStokesGenerator.batch();
		}

		if (solarPV)
		{
			double[] PV_min = MelodyStokesGenerator.PV();
			double[] rem = ArrayUtils.add(thisOneMinProfile.D_min_total, ArrayUtils.negate(PV_min));
		}

		if (solarThermal)
		{
			MelodyStokesGenerator.solar_thermal();
		}
	...
	/**
	 * 
	 * In this implementation, we will do one minute calculations on the wet
	 * appliances only
	 * 
	 * @param consumer_type
	 * @param thisHHProfile
	...
	{
		return MelodyStokesGenerator
				.one_min_wash_generate(thisHHProfile, Nspan, startDate.getYear(), thisHouse.washer_num, thisHouse.dryer_num, thisHouse.wash_dry_num, thisHouse.dish_num);
	}

	/**
	 * Translation of HH_group.m
	 * 
	 * this module runs the half-hourly demand programmes to calculate the group
	 * average demands
	 * 
	 * @param consumer_type
	 * @param thisHHProfile
	 */
	private static void HH_group(int consumer_type, MSHalfHourProfile thisHHProfile, double startDayofYear, int startDayofWeek,
			int BST_to_GMT_day, int GMT_to_BST_day, int Nspan)
	{
		if (consumer_type == 1)
		{
			// Note - we call all the methods here. In reality, we may want to
			// populate
			// only some of the arrays - for instance calculating water and heat
			// load is
			// wasted effort if we do not use them.

			// thisHHProfile.d_lights =
			// melodyStokesDomesticLightingLoadGen(startAndEnd[0],
			// startAndEnd[1]);
			// thisHHProfile.d_cook =
			// melodyStokesDomesticCookingLoadGen(startAndEnd[0],
			// startAndEnd[1]);

			MelodyStokesGenerator
					.lighting(thisHHProfile, startDayofYear, startDayofWeek, Nspan, consumer_type, BST_to_GMT_day, GMT_to_BST_day);
			MelodyStokesGenerator.cooking(thisHHProfile, startDayofYear, startDayofWeek, Nspan, consumer_type);
			MelodyStokesGenerator.cooling(thisHHProfile, startDayofYear, startDayofWeek, Nspan);
			MelodyStokesGenerator.washing(thisHHProfile, startDayofYear, startDayofWeek, Nspan);
			MelodyStokesGenerator.water(thisHHProfile, startDayofYear, Nspan);
			MelodyStokesGenerator.heat(thisHHProfile, startDayofYear, Nspan);
			MelodyStokesGenerator.misc(thisHHProfile, startDayofYear, startDayofWeek, Nspan);
			thisHHProfile = MelodyStokesGenerator.peak(thisHHProfile, null);
			// the following test out typical total half-hourly demands - using
			// default ownership of appliances

			// D_total_average_E7=D_lights+D_water_E7+D_heat+D_fridge_freezer+D_washer_E7+D_dryer_E7+D_oven+D_kettle+D_microwave+D_misc;
			double[] D_total_average_UR = ArrayUtils
					.add(thisHHProfile.D_lights, thisHHProfile.D_water_UR, thisHHProfile.D_fridge_freezer, thisHHProfile.D_washer_UR, thisHHProfile.D_dryer_UR, thisHHProfile.D_oven, thisHHProfile.D_kettle, thisHHProfile.D_microwave, thisHHProfile.D_misc);
	 ...
	 * @param nspan
	 */
	private static void lighting(MSHalfHourProfile thisProfile, double startDayOfYear, int startDayOfWeek, int Nspan, int consumer_type,
			int BST_GMT, int GMT_BST)
	{
		// this section only works for test days in a single year at present due
		// to BST / GMT stuff
		...
		// them
		// if the total is under the min level, sets demand to min; if over max
		// level, sets demand to max
		...
		if (consumer_type == 1)
		{
			...
			{
				// With this formulation, we get back to the Matlab equivalent
				// of
				...
				if (thisDayOfWeek < 7)
				{
					if (thisDayOfWeek == 1)
					{
						...
						{
							int n = 0;
							d_lights_sine1[48 * i + HH] = MSConstArrays.lighting_sinescale1_sun[HH]
									* Math.sin((2 * Math.PI * (((startDayOfYear + i) + BST) / 365) - MSConstArrays.lighting_sinephase1_sun[HH]));
							d_lights_sine2[48 * i + HH] = MSConstArrays.lighting_sinescale2_sun[HH]
									* Math.sin((2 * Math.PI * (((startDayOfYear + i) - BST) / 365) - MSConstArrays.lighting_sinephase2_sun[HH]))
									+ MSConstArrays.lighting_const_sun[HH];
							...
							{
								thisProfile.d_lights[48 * i + HH] = MSConstArrays.lighting_min_sun[HH]
										+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.lighting_stddev_min_sun[HH]);
								n = 1;
							}
							if (d_lights_sine[48 * i + HH] >= MSConstArrays.lighting_max_sun[HH])
							{
								thisProfile.d_lights[48 * i + HH] = MSConstArrays.lighting_max_sun[HH]
										+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.lighting_stddev_max_sun[HH]);
								n = 1;
							}
							if (n == 0)
							{
								thisProfile.d_lights[48 * i + HH] = d_lights_sine[48 * i + HH]
										+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.lighting_stddev_sine_sun[HH]);
							}
					...
					{
						int n = 0;
						d_lights_sine1[48 * i + HH] = MSConstArrays.lighting_sinescale1_wkdays[HH]
								* Math.sin((2 * Math.PI * (((startDayOfYear + i) + BST) / 365) - MSConstArrays.lighting_sinephase1_wkdays[HH]));
						d_lights_sine2[48 * i + HH] = MSConstArrays.lighting_sinescale2_wkdays[HH]
								* Math.sin((2 * Math.PI * (((startDayOfYear + i) - BST) / 365) - MSConstArrays.lighting_sinephase2_wkdays[HH]))
								+ MSConstArrays.lighting_const_wkdays[HH];
						d_lights_sine[48 * i + HH] = d_lights_sine1[48 * i + HH] + d_lights_sine2[48 * i + HH];
						if (d_lights_sine[48 * i + HH] <= MSConstArrays.lighting_min_wkdays[HH])
						{
							thisProfile.d_lights[48 * i + HH] = MSConstArrays.lighting_min_wkdays[HH]
									+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.lighting_stddev_min_wkdays[HH]);
							n = 1;
						}
						if (d_lights_sine[48 * i + HH] >= MSConstArrays.lighting_max_wkdays[HH])
						{
							thisProfile.d_lights[48 * i + HH] = MSConstArrays.lighting_max_wkdays[HH]
									+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.lighting_stddev_max_wkdays[HH]);
							n = 1;
						}
						if (n == 0)
						{
							thisProfile.d_lights[48 * i + HH] = d_lights_sine[48 * i + HH]
									+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.lighting_stddev_sine_wkdays[HH]);
						}
						if (thisProfile.d_lights[48 * i + HH] < 0)
						{
							thisProfile.d_lights[48 * i + HH] = 0;
				...
				if (thisDayOfWeek == 7)
				{
					...
					{
						int n = 0;
						d_lights_sine1[48 * i + HH] = MSConstArrays.lighting_sinescale1_sat[HH]
								* Math.sin((2 * Math.PI * (((startDayOfYear + i) + BST) / 365) - MSConstArrays.lighting_sinephase1_sat[HH]));
						d_lights_sine2[48 * i + HH] = MSConstArrays.lighting_sinescale2_sat[HH]
								* Math.sin((2 * Math.PI * (((startDayOfYear + i) - BST) / 365) - MSConstArrays.lighting_sinephase2_sat[HH]))
								+ MSConstArrays.lighting_const_sat[HH];
						d_lights_sine[48 * i + HH] = d_lights_sine1[48 * i + HH] + d_lights_sine2[48 * i + HH];
						if (d_lights_sine[48 * i + HH] <= MSConstArrays.lighting_min_sat[HH])
						{
							thisProfile.d_lights[48 * i + HH] = MSConstArrays.lighting_min_sat[HH]
									+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.lighting_stddev_min_sat[HH]);
							n = 1;
						}
						if (d_lights_sine[48 * i + HH] >= MSConstArrays.lighting_max_sat[HH])
						{
							thisProfile.d_lights[48 * i + HH] = MSConstArrays.lighting_max_sat[HH]
									+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.lighting_stddev_max_sat[HH]);
							n = 1;
						}
						if (n == 0)
						{
							thisProfile.d_lights[48 * i + HH] = d_lights_sine[48 * i + HH]
									+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.lighting_stddev_sine_sat[HH]);
						}
						if (thisProfile.d_lights[48 * i + HH] < 0)
						{
							thisProfile.d_lights[48 * i + HH] = 0;
		...
		for (int i = 0; i < Nspan; i++)
		{
			...
			if (thisDayOfWeek < 3)
			{
				...
				if (thisDayOfWeek == 2)
				{
					for (int HH = 0; HH < 48; HH++)
					{
						thisProfile.d_washer_UR[48 * i + HH] = MSConstArrays.scale_washer_mon_UR[HH]
								* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365)) - MSConstArrays.phase_washer_mon_UR[HH])
								+ MSConstArrays.const_washer_mon_UR[HH]
								+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_washer_mon_UR[HH]);
						thisProfile.d_dryer_UR[48 * i + HH] = MSConstArrays.scale_dryer_mon_UR[HH]
								* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365)) - MSConstArrays.phase_dryer_mon_UR[HH])
								+ MSConstArrays.const_dryer_mon_UR[HH]
								+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_dryer_mon_UR[HH]);
						thisProfile.d_dish_UR[48 * i + HH] = MSConstArrays.scale_dish_UR[HH]
								* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365)) - MSConstArrays.phase_dish_UR[HH])
								+ MSConstArrays.const_dish_UR[HH]
								+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_dish_UR[HH]);
						thisProfile.d_washer_E7[48 * i + HH] = MSConstArrays.scale_washer_mon_E7[HH]
								* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365)) - MSConstArrays.phase_washer_mon_E7[HH])
								+ MSConstArrays.const_washer_mon_E7[HH]
								+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_washer_mon_E7[HH]);
						thisProfile.d_dryer_E7[48 * i + HH] = MSConstArrays.scale_dryer_mon_E7[HH]
								* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365)) - MSConstArrays.phase_dryer_mon_E7[HH])
								+ MSConstArrays.const_dryer_mon_E7[HH]
								+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_dryer_mon_E7[HH]);
						thisProfile.d_dish_E7[48 * i + HH] = MSConstArrays.scale_dish_E7[HH]
								* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365)) - MSConstArrays.phase_dish_E7[HH])
								+ MSConstArrays.const_dish_E7[HH]
								+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_dish_E7[HH]);
				...
				if (thisDayOfWeek == 1)
				{
					for (int HH = 0; HH < 48; HH++)
					{
						thisProfile.d_washer_UR[48 * i + HH] = MSConstArrays.scale_washer_sun_UR[HH]
								* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365)) - MSConstArrays.phase_washer_sun_UR[HH])
								+ MSConstArrays.const_washer_sun_UR[HH]
								+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_washer_sun_UR[HH]);
						thisProfile.d_dryer_UR[48 * i + HH] = MSConstArrays.scale_dryer_sun_UR[HH]
								* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365)) - MSConstArrays.phase_dryer_sun_UR[HH])
								+ MSConstArrays.const_dryer_sun_UR[HH]
								+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_dryer_sun_UR[HH]);
						thisProfile.d_dish_UR[48 * i + HH] = MSConstArrays.scale_dish_UR[HH]
								* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365)) - MSConstArrays.phase_dish_UR[HH])
								+ MSConstArrays.const_dish_UR[HH]
								+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_dish_UR[HH]);
						thisProfile.d_washer_E7[48 * i + HH] = MSConstArrays.scale_washer_sun_E7[HH]
								* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365)) - MSConstArrays.phase_washer_sun_E7[HH])
								+ MSConstArrays.const_washer_sun_E7[HH]
								+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_washer_sun_E7[HH]);
						thisProfile.d_dryer_E7[48 * i + HH] = MSConstArrays.scale_dryer_sun_E7[HH]
								* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365)) - MSConstArrays.phase_dryer_sun_E7[HH])
								+ MSConstArrays.const_dryer_sun_E7[HH]
								+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_dryer_sun_E7[HH]);
						thisProfile.d_dish_E7[48 * i + HH] = MSConstArrays.scale_dish_E7[HH]
								* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365)) - MSConstArrays.phase_dish_E7[HH])
								+ MSConstArrays.const_dish_E7[HH]
								+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_dish_E7[HH]);
			...
			if (thisDayOfWeek >= 3)
			{
				...
				if (thisDayOfWeek == 7)
				{
					for (int HH = 0; HH < 48; HH++)
					{
						thisProfile.d_washer_UR[48 * i + HH] = MSConstArrays.scale_washer_sat_UR[HH]
								* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365)) - MSConstArrays.phase_washer_sat_UR[HH])
								+ MSConstArrays.const_washer_sat_UR[HH]
								+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_washer_sat_UR[HH]);
						thisProfile.d_dryer_UR[48 * i + HH] = MSConstArrays.scale_dryer_sat_UR[HH]
								* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365)) - MSConstArrays.phase_dryer_sat_UR[HH])
								+ MSConstArrays.const_dryer_sat_UR[HH]
								+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_dryer_sat_UR[HH]);
						thisProfile.d_dish_UR[48 * i + HH] = MSConstArrays.scale_dish_UR[HH]
								* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365)) - MSConstArrays.phase_dish_UR[HH])
								+ MSConstArrays.const_dish_UR[HH]
								+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_dish_UR[HH]);
						thisProfile.d_washer_E7[48 * i + HH] = MSConstArrays.scale_washer_sat_E7[HH]
								* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365)) - MSConstArrays.phase_washer_sat_E7[HH])
								+ MSConstArrays.const_washer_sat_E7[HH]
								+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_washer_sat_E7[HH]);
						thisProfile.d_dryer_E7[48 * i + HH] = MSConstArrays.scale_dryer_sat_E7[HH]
								* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365)) - MSConstArrays.phase_dryer_sat_E7[HH])
								+ MSConstArrays.const_dryer_sat_E7[HH]
								+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_dryer_sat_E7[HH]);
						thisProfile.d_dish_E7[48 * i + HH] = MSConstArrays.scale_dish_E7[HH]
								* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365)) - MSConstArrays.phase_dish_E7[HH])
								+ MSConstArrays.const_dish_E7[HH]
								+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_dish_E7[HH]);
				...
				if (thisDayOfWeek < 7)
				{
					for (int HH = 0; HH < 48; HH++)
					{
						thisProfile.d_washer_UR[48 * i + HH] = MSConstArrays.scale_washer_wkdays_UR[HH]
								* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365)) - MSConstArrays.phase_washer_wkdays_UR[HH])
								+ MSConstArrays.const_washer_wkdays_UR[HH]
								+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_washer_wkdays_UR[HH]);
						thisProfile.d_dryer_UR[48 * i + HH] = MSConstArrays.scale_dryer_wkdays_UR[HH]
								* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365)) - MSConstArrays.phase_dryer_wkdays_UR[HH])
								+ MSConstArrays.const_dryer_wkdays_UR[HH]
								+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_dryer_wkdays_UR[HH]);
						thisProfile.d_dish_UR[48 * i + HH] = MSConstArrays.scale_dish_UR[HH]
								* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365)) - MSConstArrays.phase_dish_UR[HH])
								+ MSConstArrays.const_dish_UR[HH]
								+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_dish_UR[HH]);
						thisProfile.d_washer_E7[48 * i + HH] = MSConstArrays.scale_washer_wkdays_E7[HH]
								* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365)) - MSConstArrays.phase_washer_wkdays_E7[HH])
								+ MSConstArrays.const_washer_wkdays_E7[HH]
								+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_washer_wkdays_E7[HH]);
						thisProfile.d_dryer_E7[48 * i + HH] = MSConstArrays.scale_dryer_wkdays_E7[HH]
								* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365)) - MSConstArrays.phase_dryer_wkdays_E7[HH])
								+ MSConstArrays.const_dryer_wkdays_E7[HH]
								+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_dryer_wkdays_E7[HH]);
						thisProfile.d_dish_E7[48 * i + HH] = MSConstArrays.scale_dish_E7[HH]
								* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365)) - MSConstArrays.phase_dish_E7[HH])
								+ MSConstArrays.const_dish_E7[HH]
								+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_dish_E7[HH]);
	...
	public static void cooling(MSHalfHourProfile thisProfile, double startDayOfYear, int startDayOfWeek, int Nspan)
	{
		...
		for (int i = 0; i < Nspan; i++)
		{
			for (int HH = 0; HH < 48; HH++)
			{
				thisProfile.d_fridge[48 * i + HH] = MSConstArrays.scale_fridge[HH]
						* Math.sin(2 * Math.PI * ((startDayOfYear + i) / 365) - MSConstArrays.phase_fridge[HH])
						+ MSConstArrays.const_fridge[HH] + (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_fridge[HH]);
		...
		for (int i = 0; i < Nspan; i++)
		{
			for (int HH = 0; HH < 48; HH++)
			{
				thisProfile.d_freezer[48 * i + HH] = MSConstArrays.scale_freezer[HH]
						* Math.sin(2 * Math.PI * ((startDayOfYear + i) / 365) - 2.05) + MSConstArrays.const_freezer[HH]
						+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_freezer[HH]);
		...
		for (int i = 0; i < Nspan; i++)
		{
			for (int HH = 0; HH < 48; HH++)
			{
				thisProfile.d_fridge_freezer[48 * i + HH] = MSConstArrays.scale_fridge_freezer[HH]
						* Math.sin(2 * Math.PI * ((startDayOfYear + i) / 365) - MSConstArrays.phase_fridge_freezer[HH])
						+ MSConstArrays.const_fridge_freezer[HH]
						+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_fridge_freezer[HH]);
	...
	public static void cooking(MSHalfHourProfile thisProfile, double startDayOfYear, int startDayOfWeek, int Nspan, int consumer_type)
	{
		if (consumer_type == 1)
		{
			// domestic consumers with different demand for 3 different day
			// types)
			{
				for (int i = 0; i < Nspan; i++)
				{
					// With this formulation, we get back to the Matlab
					// equivalent of
					// weekDay - i.e. 1-7 with 1 == Sunday.
					int thisDayOfWeek = (startDayOfWeek + i) % 7;
					if (thisDayOfWeek == 0)
					...
					if (thisDayOfWeek < 7)
					{
						...
						if (thisDayOfWeek == 1)
						{
							for (int HH = 0; HH < 48; HH++)
							{
								thisProfile.d_cook[48 * i + HH] = MSConstArrays.cooking_sinescale_sun[HH]
										* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365) - MSConstArrays.cooking_sinephase_sun[HH]))
										+ MSConstArrays.cooking_const_sun[HH]
										+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.cooking_stddev_sun[HH]);
						...
						if (thisDayOfWeek > 1)
						{
							for (int HH = 0; HH < 48; HH++)
							{
								thisProfile.d_cook[48 * i + HH] = MSConstArrays.cooking_sinescale_wkdays[HH]
										* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365) - MSConstArrays.cooking_sinephase_wkdays[HH]))
										+ MSConstArrays.cooking_const_wkdays[HH]
										+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.cooking_stddev_wkdays[HH]);
								if (thisProfile.d_cook[48 * i + HH] < 0)
								{
									thisProfile.d_cook[48 * i + HH] = 0;
								}
					...
					if (thisDayOfWeek == 7)
					{
						...
						for (int HH = 0; HH < 48; HH++)
						{
							thisProfile.d_cook[48 * i + HH] = MSConstArrays.cooking_sinescale_sat[HH]
									* Math.sin((2 * Math.PI * ((startDayOfYear + i) / 365) - MSConstArrays.cooking_sinephase_sat[HH]))
									+ MSConstArrays.cooking_const_sat[HH]
									+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.cooking_stddev_sat[HH]);
							if (thisProfile.d_cook[48 * i + HH] < 0)
							{
								thisProfile.d_cook[48 * i + HH] = 0;
							}
			...
			if (consumer_type == 2)
			{
				// non-domestic consumers with same demand any every day
				// (weekday domestic demand)
				for (int i = 0; i < Nspan; i++)
				{
					for (int HH = 0; HH < 48; HH++)
					{
	 ...
	 * @param nspan
	 */
	private static void misc(MSHalfHourProfile thisHHProfile, double startDayOfYear, int startDayOfWeek, int Nspan)
	{

		int consumer_type = 1; // for compatibility with old matlab code -
								// consumer_type = 1 means domestic

		// this submodel calculates a miscellaneous demand for (the group
		// average
		// sets of parameters are provided for (Mondays, Saturdays, Sundays &
		// other weekdays
		// the miscellaneous demand is assumed to apply to both E7 and
		// unrestricted tariff consumers

		double[] d_misc = new double[Nspan * 48];

		if (consumer_type == 1)
		{
			// domestic consumers (different misc demand for (int 4 different
			// day types)
			for (int i = 0; i < Nspan; i++)
			{
				// With this formulation, we get back to the Matlab equivalent
				...
				if (thisDayOfWeek < 3)
				{
					...
					if (thisDayOfWeek == 2)
					{
						...
						for (int HH = 0; HH < 48; HH++)
						{
							...
							d_misc[48 * i + HH] = MSConstArrays.scale_misc_mon[HH]
									* Math.sin((2 * Math.PI * ((startDayOfYear + i) / Consts.DAYS_PER_YEAR))
											- MSConstArrays.phase_misc_mon[HH]) + MSConstArrays.const_misc_mon[HH]
									+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_misc_mon[HH]);
							if (d_misc[48 * i + HH] < 0)
							{
					...
					if (thisDayOfWeek == 1)
					{
						...
						for (int HH = 0; HH < 48; HH++)
						{
							...
							d_misc[48 * i + HH] = MSConstArrays.scale_misc_sun[HH]
									* Math.sin((2 * Math.PI * ((startDayOfYear + i) / Consts.DAYS_PER_YEAR))
											- MSConstArrays.phase_misc_sun[HH]) + MSConstArrays.const_misc_sun[HH]
									+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_misc_sun[HH]);
							if (d_misc[48 * i + HH] < 0)
							{
				...
				if (thisDayOfWeek >= 3)
				{
					...
					if (thisDayOfWeek == 7)
					{
						...
						for (int HH = 0; HH < 48; HH++)
						{
							...
							d_misc[48 * i + HH] = MSConstArrays.scale_misc_sat[HH]
									* Math.sin((2 * Math.PI * ((startDayOfYear + i) / Consts.DAYS_PER_YEAR))
											- MSConstArrays.phase_misc_sat[HH]) + MSConstArrays.const_misc_sat[HH]
									+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_misc_sat[HH]);
							if (d_misc[48 * i + HH] < 0)
							{
					...
					if (thisDayOfWeek < 7)
					{
						...
						for (int HH = 0; HH < 48; HH++)
						{
							...
							d_misc[48 * i + HH] = MSConstArrays.scale_misc_wkdays[HH]
									* Math.sin((2 * Math.PI * ((startDayOfYear + i) / Consts.DAYS_PER_YEAR))
											- MSConstArrays.phase_misc_wkdays[HH]) + MSConstArrays.const_misc_wkdays[HH]
									+ (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_misc_wkdays[HH]);
							if (d_misc[48 * i + HH] < 0)
							{
		...
		if (consumer_type == 2)
		{
			 ...
			 * for (int HH=0; HH < 48; HH++) {
			 * 
			 * d_misc(i,HH)=MSConstArrays.scale_misc_wkdays[HH]*sin((2*pi*(N(i)/
			 * Ndays(1)))-MSConstArrays.phase_misc_wkdays[HH])+MSConstArrays.
			 * const_misc_wkdays
			 * [HH]+(randn*MSConstArrays.stddev_misc_wkdays[HH]); if
			 * (d_misc(i,HH) <0) {
			 * 
			 * d_misc(i,HH) = 0; } } }
			 */
	 ...
	 * @param nspan
	 */
	private static void heat(MSHalfHourProfile thisHHProfile, double startDayofYear, int Nspan)
	{
		// this sub-model calculates space heating (storage heaters) for a group
		// average of E7 consumers
		...
		for (int i = 0; i < Nspan; i++)
		{
			for (int HH = 0; HH < 48; HH++)
			{
				d_heat[48 * i + HH] = MSConstArrays.scale_heat[HH]
						* Math.sin((2 * Math.PI * ((startDayofYear + i) / Consts.DAYS_PER_YEAR)) - MSConstArrays.phase_heat[HH])
						+ MSConstArrays.const_heat[HH];
	...
	{
		// this sub-model calculates the half-hourly group average demand for
		...
		for (int i = 0; i < Nspan; i++)
		{
			for (int HH = 0; HH < 48; HH++)
			{
				...
				thisHHProfile.d_water_UR[48 * i + HH] = MSConstArrays.scale_water_UR[HH]
						* Math.sin((2 * Math.PI * ((startDayOfYear + i) / Consts.DAYS_PER_YEAR)) - MSConstArrays.phase_water_UR[HH])
						+ MSConstArrays.const_water_UR[HH] + (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_water_UR[HH]);
				thisHHProfile.d_water_E7[48 * i + HH] = MSConstArrays.scale_water_E7[HH]
						* Math.sin((2 * Math.PI * ((startDayOfYear + i) / Consts.DAYS_PER_YEAR)) - MSConstArrays.phase_water_E7[HH])
						+ MSConstArrays.const_water_E7[HH] + (RandomHelper.getNormal().nextDouble() * MSConstArrays.stddev_water_E7[HH]);
	...
	private static MSHalfHourProfile peak(MSHalfHourProfile thisHHProfile, Date startDate)
	{
		...
		// it begins by working out the scaling factors for each end-use demand
		// (annual peak)in kW
		// the trends have a different number of years in the datasets (11,12
		// and 7)
		// the annual peak demand is interpolated linearly between the data
		// values
		// the routines below find where the test year falls in relation to the
		// data values
		...
		double b = (double) (y - MSConstArrays.trend_year_11[a - 1])
				/ (MSConstArrays.trend_year_11[a] - MSConstArrays.trend_year_11[a - 1]); // interpolate...
		double lighting_peak = MSConstArrays.lighting_trend[a - 1] + b
				* (MSConstArrays.lighting_trend[a] - MSConstArrays.lighting_trend[a - 1]);
		...
		double water_E7_peak = MSConstArrays.water_trend_E7[a - 1] + b
				* (MSConstArrays.water_trend_E7[a] - MSConstArrays.water_trend_E7[a - 1]);
		double water_UR_peak = MSConstArrays.water_trend_UR[a - 1] + b
				* (MSConstArrays.water_trend_UR[a] - MSConstArrays.water_trend_UR[a - 1]);
		double fridge_peak = MSConstArrays.fridge_trend[a - 1] + b * (MSConstArrays.fridge_trend[a] - MSConstArrays.fridge_trend[a - 1]);
		double freezer_peak = MSConstArrays.freezer_trend[a - 1] + b
				* (MSConstArrays.freezer_trend[a] - MSConstArrays.freezer_trend[a - 1]);
		double fridge_freezer_peak = MSConstArrays.fridge_freezer_trend[a - 1] + b
				* (MSConstArrays.fridge_freezer_trend[a] - MSConstArrays.fridge_freezer_trend[a - 1]);
		...
		double water_own_peak = MSConstArrays.water_own[a - 1] + b * (MSConstArrays.water_own[a] - MSConstArrays.water_own[a - 1]); // Note
																																	// -
																																	// mistake
																																	// in
																																	// original
		...
		double microwave_peak = MSConstArrays.microwave_trend[a - 1] + b
				* (MSConstArrays.microwave_trend[a] - MSConstArrays.microwave_trend[a - 1]);
		double oven_hob_ratio = MSConstArrays.oven_hob_trend[a - 1] + b
				* (MSConstArrays.oven_hob_trend[a] - MSConstArrays.oven_hob_trend[a - 1]);

		double heat_peak = (((double) y - 1996) / 4) * (7.649 - 5.811) + 5.811;

		// having established the annual peak value, the annual trends in demand
		// for each end-use are scaled accordingly
		...
		thisHHProfile.D_washer_E7 = ArrayUtils.multiply(thisHHProfile.d_washer_E7, washer_peak * 0.180 / 0.235); // takes
																													// account
																													// of
																													// different
																													// peak
		...
		thisHHProfile.D_dryer_E7 = ArrayUtils.multiply(thisHHProfile.d_dryer_E7, dryer_peak * 0.309 / 0.347); // takes
																												// account
																												// of
																												// different
																												// peak
		...
		thisHHProfile.D_dish_E7 = ArrayUtils.multiply(thisHHProfile.d_dish_E7, dish_peak * 0.411); // different
																									// LRG
																									// peak
																									// values
	...
	private static void HH_specific(MSHouseholdCharacteristics thisHousehold, boolean MSownershipCalc, MSHalfHourProfile thisProfile,
			Date startDate, int Nspan)
	{
		...
		// Economy 7 tariffs (EY) and, in the case of unknown, either is
		// selected.
		/*
		 * Original Matlab code for tariff selection.
		 * 
		 * str = {'Unrestricted' 'Off-peak,eg E7' 'Unknown'};
		 * [tar,v]=listdlg('PromptString','Select tariff','ListString',str,
		 * 'SelectionMode','single'); if (tar == 3 R=rand if (R < 0.25 tar = 2 }
		 * if (R >= 0.25 tar = 1 } }
		 */

		// We fix tariff as UR as we are interested in deviations from the
		// "norm"
		// or unrestricted tariff.
		// TODO: check this assumption with others on the project.
		int tar = 1;

		// In the original Matlab model, ownership calculates the ownership of
		// various appliances.
		// We can either use this or the ownership as passed into the function
		// in this implementation.
		if (MSownershipCalc)
		{
			thisHousehold = MelodyStokesGenerator.ownership();
		}
		...
		{// space heating required
			thisProfile.D_HHspecific_heat = ArrayUtils.multiply(thisProfile.D_heat, (thisHousehold.floor_area / 103.2)); // scaled
																															// by
																															// floor
																															// area,
																															// average
		...
		if (thisHousehold.num_occ <= 7)
		{
			water_scale_value1 = MSConstArrays.water_scale[thisHousehold.num_occ]; // selects
																					// weighting
																					// from
																					// array
		}
		...
		water_scale_value = water_scale_value1 * 0.6; // randomly scales water
														// demand based on
		...
		else if (thisHousehold.water_marker == 2)
		{
			...
			for (int i = 0; i < Nspan; i++)
			{
				// Original matlab below - N_serial is an array of serial
				// numbers of every
				// date in the profile being produced.
				// p=datevec(N_serial(i)) ;// p is date matrix with 2nd element
				// being month number
			...
			thisProfile.D_HHspecific_cook = ArrayUtils
					.add(thisProfile.D_HHspecific_hob, thisProfile.D_HHspecific_oven, thisProfile.D_HHspecific_microwave, thisProfile.D_HHspecific_kettle);
			...
			if (thisHousehold.fridge_marker)
			{
				...
				{
					thisProfile.D_HHspecific_fridge = ArrayUtils
							.multiply(thisProfile.D_fridge, MSConstArrays.cool_scale[thisHousehold.num_occ]); // assumes
																												// average
																												// oocupancy
																												// of
																												// 2.4
			...
			if (thisHousehold.ff_marker)
			{
				if (thisHousehold.num_occ <= 5)
				{
					thisProfile.D_HHspecific_ff = ArrayUtils
							.multiply(thisProfile.D_fridge_freezer, MSConstArrays.cool_scale[thisHousehold.num_occ] * thisHousehold.ff_num); // assumes
																																				// average
																																				// oocupancy
																																				// of
																																				// 2.4
				...
				if (thisHousehold.num_occ > 5)
				{
					thisProfile.D_HHspecific_ff = ArrayUtils.multiply(thisProfile.D_fridge_freezer, MSConstArrays.cool_scale[5]
							* thisHousehold.ff_num);
				}
			}
			else
			{
			...
			if (thisHousehold.freezer_marker)
			{
				...
				{
					R = RandomHelper.getNormal().nextDouble() * 0.316; // normally
																		// distributed
																		// random
																		// number
																		// with
																		...
																		// of
																		// 0.316
					thisProfile.D_HHspecific_freezer = ArrayUtils
							.add(ArrayUtils.multiply(thisProfile.D_freezer, R), thisProfile.D_HHspecific_freezer);
				}
			}

			thisProfile.D_HHspecific_cold = ArrayUtils
					.add(thisProfile.D_HHspecific_fridge, thisProfile.D_HHspecific_ff, thisProfile.D_HHspecific_freezer);
			...
			{
				wash_scale_value = thisHousehold.num_occ * 1.7 / 4.77; // based
																		// on
																		// 1.7
																		// washes/person
																		// for 6
			...
			thisProfile.D_HHspecific_washing = ArrayUtils
					.add(thisProfile.D_HHspecific_wash, thisProfile.D_HHspecific_dryer, thisProfile.D_HHspecific_dish);
			...
			for (int i = 0; i < Nspan; i++)
			{
				for (int j = 0; j < 48; j++)
				{
					thisProfile.D_HHspecific_lights[i * 48 + j] = thisProfile.D_lights[i * 48 + j]
							* MSConstArrays.light_occ_scale[j][num_occ_temp];
					thisProfile.D_HHspecific_misc[i * 48 + j] = thisProfile.D_misc[i * 48 + j]
							* MSConstArrays.light_occ_scale[j][num_occ_temp];
				}
			}
			...
			if (thisHousehold.income_num == 0)
			{
				...
				for (int i = 0; i < Nspan; i++)
				{
					for (int j = 0; j < 48; j++)
					{
						thisProfile.D_HHspecific_lights[48 * i + j] = thisProfile.D_HHspecific_lights[48 * i + j]
								* MSConstArrays.light_social_scale[j][social_num_value];
						thisProfile.D_HHspecific_misc[48 * i + j] = thisProfile.D_HHspecific_misc[48 * i + j]
								* MSConstArrays.light_social_scale[j][social_num_value];
					}
				}
			}
			...
			if (thisHousehold.income_num > 0)
			{
				...
				for (int i = 0; i < Nspan; i++)
				{
					...
					{
						thisProfile.D_HHspecific_lights[48 * i + j] = thisProfile.D_HHspecific_lights[48 * i + j]
								* MSConstArrays.light_income_scale[j][thisHousehold.income_num];
						thisProfile.D_HHspecific_misc[48 * i + j] = thisProfile.D_HHspecific_misc[48 * i + j]
								* MSConstArrays.light_income_scale[j][thisHousehold.income_num];
					}
				}
			...
			thisProfile.D_HHspecific_total = ArrayUtils
					.add(thisProfile.D_HHspecific_heat, thisProfile.D_HHspecific_water, thisProfile.D_HHspecific_cook, thisProfile.D_HHspecific_cold, thisProfile.D_HHspecific_washing, thisProfile.D_HHspecific_lights, thisProfile.D_HHspecific_misc);

			// calculates the half-hourly specific reactive component
			// assigns PF as follows:
			// heating =1; water heat=1; lights=1;
			// cooling=0.7;cooking=1.0;washing=0.93/drying=0.98/dishwashing=1;misc.=1.0
			// reactive load = active load*tan(phi), where phi=cos-1(PF)

			thisProfile.D_HHreactive_total = ArrayUtils.add(ArrayUtils.multiply(thisProfile.D_HHspecific_cold, 1.02), ArrayUtils
					.multiply(thisProfile.D_HHspecific_wash, 0.40), ArrayUtils.multiply(thisProfile.D_HHspecific_dryer, 0.20));

			for (int i = 0; i < Nspan; i++)
			{
				for (int j = 0; j < 48; j++)
				{
					thisProfile.PF_HH[48 * i + j] = Math.cos(Math.atan(thisProfile.D_HHreactive_total[48 * i + j]
							/ thisProfile.D_HHspecific_total[48 * i + j]));
				}
			}
		}
	 ...
	 * provide a household which owns no appliances, with average socio-economic
	 * and physical characteristics and an occupancy of 2.
	 * 
	 * @return
	 * 
	 */
	...
	/**
	 * Rather a place holder at the moment. In the original (dates_1.m), this
	 * had user dialogues to determine the dates between which the profiles
	 * should be generated. For us now, this simply does a year based upon a
	 * given date. There may well be better ways...
	 * 
	 * @return
	 */
	private static Date[] dates_1()
	{
		...
		// If the consumer type was non-domestic - these were forced to be a
		// whole and single year
		// which was set to be the year of dstart (i.e. dend =
		// Date(dstart.getYear(),12,31)
		...
		// Ndays - number of days in the start year (element 1) and end year
		// (element 2)
		...
		// BST_GMT - day of start year that clocks go from BST to GMT (i.e. last
		// Sunday in October)
		// GMT_BST - day of start year that clocks go from GMT_TO BST (last
		// sunday in March)
	...
	private static double[] one_min_profile(MSHalfHourProfile thisProfile, int startDayOfWeek, int time_leaveHH, int time_returnHH,
			MSHouseholdCharacteristics thisHouse, Date start)
	{

		int Nspan = thisProfile.D_HHspecific_wash.length;
		// sets all specific half-hourly demands for end-uses other than
		// fridges, etc. + heating to zero
		...
		return MelodyStokesGenerator.one_min(thisHouse, thisProfile, start, Nspan);
	}

	private static double[] one_min_wash_generate(MSHalfHourProfile thisHHProfile, int Nspan, int y, int washMachs, int dryers,
			int washer_dryers, int dishwashers)// y = year of simulation
	{
		...
		double b = (y - MSConstArrays.trend_year_wash[a - 1]) / (MSConstArrays.trend_year_wash[a] - MSConstArrays.trend_year_wash[a - 1]);
		...
		double dish_temp = MSConstArrays.dish_temp_trend[a - 1] + b
				* (MSConstArrays.dish_temp_trend[a] - MSConstArrays.dish_temp_trend[a - 1]);
		if (y < 1990)
		{
			dish_temp = 0.68;
		}
