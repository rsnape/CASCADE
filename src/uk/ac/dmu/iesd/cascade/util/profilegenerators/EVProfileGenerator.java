	...
	public static double[] generateBEVProfile(CascadeContext context, int days)
	{
		...
		for (int i = 0; i < days; i++)
		{
			if (RandomHelper.nextDouble() < Consts.JOURNEY_MADE_PROBABILITY)
			{
				...
				double requiredCharge = journeyLength * EVProfileGenerator.BEVconsumption;
				if (requiredCharge > EVProfileGenerator.BEVcapacity)
				{
					requiredCharge = EVProfileGenerator.BEVcapacity;
				}
	...
	public static double[] generatePHEVProfile(CascadeContext context, int days)
	{
		...
		for (int i = 0; i < days; i++)
		{
			if (RandomHelper.nextDouble() < Consts.JOURNEY_MADE_PROBABILITY)
			{
				...
				double requiredCharge = journeyLength * EVProfileGenerator.PHEVconsumption;
				if (requiredCharge > EVProfileGenerator.PHEVcapacity)
				{
					requiredCharge = EVProfileGenerator.PHEVcapacity;
				}
				double chargePeriod = context.ticksPerDay - context.ticksPerDay + 14;
