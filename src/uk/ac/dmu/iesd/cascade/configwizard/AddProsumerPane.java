...
public class AddProsumerPane extends WizardWorkingPane implements ActionListener
{
	 ...
	 */
	public AddProsumerPane(Document thisConfig, Element currentAggregator2)//, AggregatorAgent agg)
	{
		super();
		...
		proList = new JComboBox(WizardFrame.prosumerClasses.toArray());
		//aggList.setMinimumSize(new Dimension(400,25));
		proList.setBounds(144, 0, line1.getWidth()-170, 25);
	...
	public boolean validateAndSaveToConfig()
	{
		...
		if (proList.getSelectedIndex() >= 0)
		{
			selectedProsumer = (Class<? extends ProsumerAgent>) proList.getSelectedItem();
		}
		else
		{
			validatesOK = false;
		...
		{	
			...
			Element proElement = configObject.createElement("prosumer");
			...
			this.mainContext.logger.debug("Added element " + proElement.toString() + " as an child element of " + getWorkingElement().toString());
