...
public class CascadeContextBuilder implements ContextBuilder<Object>
{
	...
	private void readParamsAndInitializeArrays()
	{
		...
		this.cascadeMainContext.setGasPercentage(this.percentageOfHHProsWithGas);
		...
		try
		{
			...
			this.cascadeMainContext.insolationArray = Arrays.copyOf(insolationArray_all, insolationArray_all.length);
			this.cascadeMainContext.windSpeedArray = Arrays.copyOf(windSpeedArray_all, windSpeedArray_all.length);
			this.cascadeMainContext.airTemperatureArray = Arrays.copyOf(airTemperatureArray_all, airTemperatureArray_all.length);
			this.cascadeMainContext.airDensityArray = Arrays.copyOf(airDensityArray_all, airDensityArray_all.length);

			this.cascadeMainContext.weatherDataLength = windSpeedArray_all.length;// lengthOfProfileArrays;
	...
	private void createWindFarmsAndAddThemToContext()
	{
		...
		for (int numWindFarms = 0; numWindFarms < 1500; numWindFarms++)
		{
			...
			double[] minWindSpeed = new double[]
			{ 2.5, 2.5, 2.5, 2.5, 2.5, 2.5, 2.5, 2.5, 2.5, 2.5, 2.5, 2.5, 2.5, 2.5, 2.5, 2.5, 2.5, 2.5, 2.5, 2.5 };
			double[] maxWindSpeed = new double[]
			{ 25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 25 };
			double[] capacity = new double[]
			{ 2.0E6, 2.0E6, 2.0E6, 2.0E6, 2.0E6, 2.0E6, 2.0E6, 2.0E6, 2.0E6, 2.0E6, 2.0E6, 2.0E6, 2.0E6, 2.0E6, 2.0E6, 2.0E6, 2.0E6, 2.0E6,
					2.0E6, 2.0E6 };
			double[] bladeLength = new double[]
			{ 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40 };
			genProsAgent.setUpWindFarm(50.0, 0.0, 20, 1, 1, hubHeights, cp, minWindSpeed, maxWindSpeed, capacity, bladeLength);

			if (!this.cascadeMainContext.add(genProsAgent))
			{
	...
	private void createHouseholdProsumersAndAddThemToContext(int occupancyModeOrNb)
	{
		...
		for (int i = 0; i < nbOfHHProsWithElectricty; i++)
		{
			...
			HouseholdProsumer hhProsAgent = prosumerFactory
					.createHouseholdProsumer(this.map_nbOfOccToOtherDemand, occupancyModeOrNb, true, false);
			hhProsAgent.initializeWaterHeatDemand();
		...
		RaspPiHousehold oneOffPiHH = new RaspPiHousehold(this.cascadeMainContext, this.map_nbOfOccToOtherDemand.get(2));
		if (!this.cascadeMainContext.add(oneOffPiHH))
		{
		...
		else
		{
			if (this.cascadeMainContext.logger.isDebugEnabled())
			{
				this.cascadeMainContext.logger.debug("Put the one of Pi agent into the context");
			}
		}
		...
		this.setWetAppsPerPBMatlabPrototype(oneOffPiHH);
		oneOffPiHH.hasRefrigerator = true;

		oneOffPiHH
				.setColdAppliancesProfiles(InitialProfileGenUtils
						.melodyStokesColdApplianceGen(Consts.NB_OF_DAYS_LOADED_DEMAND, oneOffPiHH.hasRefrigerator, oneOffPiHH.hasFridgeFreezer, (oneOffPiHH.hasUprightFreezer || oneOffPiHH.hasChestFreezer)));

		oneOffPiHH.setHasElectricalSpaceHeat(true);
		oneOffPiHH.initializeElecSpaceHeatPar();
		oneOffPiHH.setHasElectricalWaterHeat(true);
		oneOffPiHH.initializeWaterHeatDemand();
	...
	private void buildSocialNetwork()
	{
		...
		Network socialNet = networkFactory.createNetwork("socialNetwork", this.cascadeMainContext, gen, directed);
		 ...
		 */
		Query<HouseholdProsumer> cat1Query = new PropertyEquals(this.cascadeMainContext, "defraCategory", 1);
	...
	private void buildOtherNetworks(AggregatorAgent firstAggregator)
	{
		...
		/*
		 * Create the projections needed in the context and add agents to those
		 * projections
		 */
		GeographyParameters geoParams = new GeographyParameters();
		Geography geography = GeographyFactoryFinder.createGeographyFactory(null)
				.createGeography("Geography", this.cascadeMainContext, geoParams);
		...
		Network economicNet = networkFactory.createNetwork("economicNetwork", this.cascadeMainContext, directed);
		...
		Network physicalNet = networkFactory.createNetwork("electricalNetwork", this.cascadeMainContext, directed);
		// TODO: How does info network differ from economic network?
		Network infoNet = networkFactory.createNetwork("infoNetwork", this.cascadeMainContext, directed);
	...
	/**
	 * This method builds the schedules directly/manually. The motivation behind
	 * its built was related to the reported bug in the random seed, and hence
	 * order of scheduling and reproducibility. It is not currently in used.
	 */
	private void buildSchedulesDirectly()
	{
		...
		IndexedIterable<ProsumerAgent> prosumerIndIter = this.cascadeMainContext.getObjects(ProsumerAgent.class);
		...
		IndexedIterable<AggregatorAgent> aggregatorIndIter = this.cascadeMainContext.getObjects(AggregatorAgent.class);
	...
	/*
	 * private void buildNetworkOfRegisteredTraders() { //boolean directed =
	 * true; NetworkFactory networkFactory =
	 * NetworkFactoryFinder.createNetworkFactory(null); Network bmuNet =
	 * networkFactory.createNetwork("RegisteredBMUsNetwork", cascadeMainContext,
	 * true); for (IPxTrader bmuAgent:(Iterable<IPxTrader>)
	 * (cascadeMainContext.getObjects(IPxTrader.class))) { bmuNet.addEdge(sysOp,
	 * bmuAgent); } cascadeMainContext.setNetworkOfRegisteredPxTraders(bmuNet);
	 * }
	 */
	 ...
	 * FactoryFinder.createAggregatorFactory(this.cascadeMainContext); RECO
	 * firstRecoAggregator =
	 * aggregatorFactory.createRECO(cascadeMainContext.systemPriceSignalDataArray
