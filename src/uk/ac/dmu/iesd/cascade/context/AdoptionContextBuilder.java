...
public class AdoptionContextBuilder implements ContextBuilder<Household>
{
	...
	public Context build(Context context)
	{
		...
		this.myContext = new AdoptionContext(context, "01/04/2010");
		...
		households = IterableUtils.Iterable2ArrayList(this.myContext.getObjects(Household.class));
		...
		for (Household thisHousehold : households)
		{
			if (fromShapefile)
			{
				...
				this.setOptions(thisHousehold, numOfOccupant, false);
...
if (			this.myContext.logger.isDebugEnabled()) {
	...
	void populateSyntheticGeography(int numHouseholds, DefaultGeography<Household> geog)
	{
		...
		{
			Household thisHousehold = this.createHouseholdProsumer(this.map_nbOfOccToOtherDemand, Consts.RANDOM, true, false);// (RandomHelper.nextDouble()
																																// <
																																// 0.7));
	...
	{
		IndexedIterable<Household> householdProsumers = this.myContext.getObjects(Household.class);
...
if (	 * this.myContext.logger.isTraceEnabled()) {
	...
	private void initializeWithoutGasHHProsumersElecSpaceHeat()
	{
		...
		IndexedIterable<Household> spaceHeatedProsumersIter = this.myContext.getObjects(Household.class);
	...
	Household createHouseholdProsumer(WeakHashMap<Integer, double[]> map_nbOfOcc2OtherDemand, int occupancyModeOrNb, boolean addNoise,
			boolean hasGas)
	{
	...
	Household createHouseholdProsumer(double[] otherDemandProfileArray, int numOfOccupants, boolean addNoise, boolean hasGas)
	{
		...
		if (addNoise)
		{
			hhProsAgent = new Household(this.myContext, this.randomizeDemandProfile(otherDemandProfileArray));
		}
		else
		{
			hhProsAgent = new Household(this.myContext, otherDemandProfileArray);
		}
