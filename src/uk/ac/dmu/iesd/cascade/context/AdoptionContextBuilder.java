...
public class AdoptionContextBuilder implements ContextBuilder<Household> {
	...
	{	
		myContext = new AdoptionContext(context,"01/07/2009");
	    ...
	    */ 
		...
		if (myContext.weatherDataLength % 48 != 0)
		{
		...
		{
			Household thisHousehold = new Household(myContext);
...
/*		if (writeInitialShapefile) {
	...
	//	myContext.verbose = (Boolean) params.getValue("verboseOutput");
		//myContext.chartSnapshotOn = (Boolean) params.getValue("chartSnapshot");
		//myContext.setChartSnapshotInterval((Integer) params.getValue("chartSnapshotInterval"));
		...
		/*try {
			...
			myContext.systemPriceSignalDataArray = Arrays.copyOf(systemBasePriceSignal, lengthOfProfileArrays);
		...
		if (myContext.systemPriceSignalDataLength % myContext.getNbOfTickPerDay()!= 0)
		{
...
/*		if(myContext.verbose)
		{
			System.out.println("HHs with Fridge: " + (double) IterableUtils.count((new PropertyEquals(myContext, "hasRefrigerator",true)).query()));
			System.out.println("HHs with FridgeFreezer: " + (double) IterableUtils.count((new PropertyEquals(myContext, "hasFridgeFreezer",true)).query()));
			System.out.println("HHs with UprightFreezer: " + (double) IterableUtils.count((new PropertyEquals(myContext, "hasUprightFreezer",true)).query()));
			System.out.println("HHs with ChestFreezer: " + (double) IterableUtils.count((new PropertyEquals(myContext, "hasChestFreezer",true)).query()));
	
			System.out.println("HHs with Fridge %: " + (double) IterableUtils.count((new PropertyEquals(myContext, "hasRefrigerator",true)).query()) / householdProsumers.size());
			System.out.println("HHs with FridgeFreezer %: " + (double) IterableUtils.count((new PropertyEquals(myContext, "hasFridgeFreezer",true)).query()) / householdProsumers.size());
			System.out.println("HHs with UprightFreezer %: " + (double) IterableUtils.count((new PropertyEquals(myContext, "hasUprightFreezer",true)).query()) / householdProsumers.size());
			System.out.println("HHs with ChestFreezer %: " + (double) IterableUtils.count((new PropertyEquals(myContext, "hasChestFreezer",true)).query()) / householdProsumers.size());
		}*/
	}
	...
	private void initializeHHProsumersWetAppliancesPar4All() {
		...
		/*if(myContext.verbose)
		{
			...
			System.out.println("households with occupancy 1 : " + (double) IterableUtils.count((new PropertyEquals(myContext, "numOccupants",1)).query()) / householdProsumers.size());
			System.out.println("households with occupancy 2 : " + (double) IterableUtils.count((new PropertyEquals(myContext, "numOccupants",2)).query()) / householdProsumers.size());
			System.out.println("households with occupancy 3 : " + (double) IterableUtils.count((new PropertyEquals(myContext, "numOccupants",3)).query()) / householdProsumers.size());
			System.out.println("households with occupancy 4 : " + (double) IterableUtils.count((new PropertyEquals(myContext, "numOccupants",4)).query()) / householdProsumers.size());
			System.out.println("households with occupancy 5 : " + (double) IterableUtils.count((new PropertyEquals(myContext, "numOccupants",5)).query()) / householdProsumers.size());
			System.out.println("households with occupancy 6 : " + (double) IterableUtils.count((new PropertyEquals(myContext, "numOccupants",6)).query()) / householdProsumers.size());
			System.out.println("households with occupancy 7 : " + (double) IterableUtils.count((new PropertyEquals(myContext, "numOccupants",7)).query()) / householdProsumers.size());
			System.out.println("households with occupancy 8 : " + (double) IterableUtils.count((new PropertyEquals(myContext, "numOccupants",8)).query()) / householdProsumers.size());
			System.out.println("Washing Mach : " + (double) IterableUtils.count((new PropertyEquals(myContext, "hasWashingMachine",true)).query()) / householdProsumers.size());
			System.out.println("Washer Dryer : " + (double) IterableUtils.count((new PropertyEquals(myContext, "hasWasherDryer",true)).query()) / householdProsumers.size());
			System.out.println("Tumble Dryer: " + (double) IterableUtils.count((new PropertyEquals(myContext, "hasTumbleDryer",true)).query()) / householdProsumers.size());
			System.out.println("Dish Washer : " + (double) IterableUtils.count((new PropertyEquals(myContext, "hasDishWasher",true)).query()) / householdProsumers.size());
		}*/
	}
	...
	private void initializeProbabilityDistributions() {
		...
		//ChartUtils.testProbabilityDistAndShowHistogram(myContext.wetApplProbDistGenerator, 10000, 48);  //test to make sure the prob dist generate desired outcomes
	
		//myContext.hhProsumerElasticityTest = RandomHelper.createBinomial(1, 0.005);
	...
	private void initializeHHProsumersElecWaterHeat() {
		...
		//Iterable waterHeatedProsumersIter = myContext.getRandomObjects(Household.class, (long) (numProsumers * (Double) params.getValue("elecWaterFraction")));
		Iterable waterHeatedProsumersIter = myContext.getRandomObjects(Household.class, (long) (numProsumers));
		...
		while (iter.hasNext()) {
			((Household) iter.next()).initializeElectWaterHeatPar();
		}
	} */
	...
	/*private void populateContext_test() {
			...
			AggregatorFactory aggregatorFactory = FactoryFinder.createAggregatorFactory(this.myContext);
			RECO firstRecoAggregator = aggregatorFactory.createRECO(myContext.systemPriceSignalDataArray);
		...
		private void populateContext() {
			 ...
			 * (02/07/12) DF
			 * 
			 * Add a simple single non domestic aggregator into <code>myContext</code>
			 */
	//		SingleNonDomesticAggregator singleNonDomestic = aggregatorFactory.createSingleNonDomesticAggregator(myContext.systemPriceSignalDataArray);
