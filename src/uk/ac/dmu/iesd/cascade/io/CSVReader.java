...
public class CSVReader
{
	...
	public void parseRaw()
	{
		...
		while (hasNextLine)
		{
			...
			{
				this.contentsByColumn.put(this.colHeaders[k], this.dataArray[k].toArray(new String[this.numRows]));
			}
		}

		Logger.getLogger(Consts.CASCADE_LOGGER_NAME).info("Parsed file " + this.CSVFileName + ": " + this.numCols + " columns and "
				+ this.numRows + " rows.");
	}
	...
	public void parseByColumn()
	{
		...
		else
		{
			...
			StringTokenizer myTokenizer = new StringTokenizer(thisLine, this.mySeperator);
	...
	public String[] getColumn(String colName)
	{
		...
		else
		{
			...
			System.err.println("Number of columns is " + this.numCols + " with names " + Arrays.toString(this.colHeaders));
		}
	...
	{
		this(new FileReader(myFilename), seperator, CSVReader.DEFAULT_HAS_COL_HEADERS);
		this.CSVFileName = myFilename;
	}

	public CSVReader(File myFile, String seperator) throws FileNotFoundException
	{
		this(new FileReader(myFile), seperator, CSVReader.DEFAULT_HAS_COL_HEADERS);
