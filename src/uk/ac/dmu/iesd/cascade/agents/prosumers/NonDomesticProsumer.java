...
public class NonDomesticProsumer extends ProsumerAgent
{
	...
	public void step()
	{
		...
		this.checkWeather(time);

		// Do all the "once-per-day" things here
		if (timeOfDay == 0)
		{
			this.inelasticTotalDayDemand = this.calculateFixedDayTotalDemand(time);
			if (this.hasSmartControl)
			{
				this.smartControlLearn(time);
			}
		}

		if (this.hasSmartControl)
		{
			this.setNetDemand(this.smartDemand(time));
		}
		else if (this.hasSmartMeter && this.exercisesBehaviourChange)
		{
			this.learnBehaviourChange();
			this.setNetDemand(this.evaluateBehaviour(time));
			this.learnSmartAdoptionDecision(time);
		}
		else
		{
			// No adaptation case
			this.setNetDemand(this.arr_otherDemandProfile[time % this.arr_otherDemandProfile.length] - this.currentGeneration());
	...
	private double windGeneration()
	{
		if (this.hasWind)
		{
			...
			return (Math.max((Math.min(this.getWindSpeed(), 25) - 5), 0)) / 20 * this.ratedPowerWind;
		}
		else
		{
			return 0;
	...
	/*
	 * Evaluates the net demand mediated by the prosumers behaviour in a given
	 * half hour.
	 * 
	...
	private double evaluateBehaviour(int time)
	{
		...
		{
			double predictedCostNow = this.getPredictedCostSignal()[timeSinceSigValid % this.getPredictedCostSignalLength()];
			if (predictedCostNow > this.costThreshold)
			{
...
if (				this.mainContext.logger.isDebugEnabled()) {
	...
	{
		// Very simple function at the moment - just return the smart profile
		// for this time
		// Previously defined by smartControlLearn()
		double myDemand;
		myDemand = this.smartOptimisedProfile[time % this.smartOptimisedProfile.length];
		return myDemand;
	}
	...
	{
		// simplest smart controller implementation - perfect division of load
		// through the day
		...
		{
			Arrays.fill(daysOptimisedDemand, this.inelasticTotalDayDemand / this.mainContext.ticksPerDay);
			movedThisTime = 0;
...
if (			this.mainContext.logger.isDebugEnabled()) {
			...
			for (Object thisConn : socialConnections)
			{
				...
				if (((NonDomesticProsumer) myConn.getSource()).hasSmartControl)
				{
					...
					inwardInfluence = inwardInfluence + myConn.getWeight()
							* ((NonDomesticProsumer) myConn.getSource()).transmitPropensitySmartControl;
				}
			}
		}
	...
	public NonDomesticProsumer(CascadeContext context, double[] baseDemand)
	{
		...
		this.setElasticityFactor(this.percentageMoveableDemand);
