...
/**
 * This class is a factory for creating instances of <tt>ProsumerAgent</tt>
 * concrete subclasses Its public creator method's signatures are defined by
 * {@link IProsumerFactory} interface.
 * 
...
public class ProsumerFactory implements IProsumerFactory
{
	...
	public HouseholdProsumer createHouseholdProsumer(double[] otherDemandProfileArray, int numOfOccupants, boolean addNoise, boolean hasGas)
	{
		...
		int ticksPerDay = this.cascadeMainContext.getNbOfTickPerDay();
		...
		if (addNoise)
		{
			hhProsAgent = new HouseholdProsumer(this.cascadeMainContext, this.randomizeDemandProfile(otherDemandProfileArray));
		}
		else
		{
			hhProsAgent = new HouseholdProsumer(this.cascadeMainContext, otherDemandProfileArray);
		}
	...
	public HouseholdProsumer createHouseholdProsumer(WeakHashMap<Integer, double[]> map_nbOfOcc2OtherDemand, int occupancyModeOrNb,
			boolean addNoise, boolean hasGas)
	{
		...
		{
			numOfOccupant = this.cascadeMainContext.occupancyGenerator.nextInt() + 1;
		...
		if (this.cascadeMainContext.signalMode == Consts.SIGNAL_MODE_SMART)
		{
			/*
	...
	/*
	 * Creates a prosumer to represent a pure generator. Therefore zero base
	 * demand, set the generator type and capacity.
	 * 
	 * TODO: should the base demand be zero or null???
	 * 
	 * @param Capacity - the generation capacity of this agent (kWh)
	 * 
	 * @param type - the type of this generator (from an enumerator of all
	 * types)
	 */
	...
	public ProsumerAgent createPureGenerator(double capacity, Consts.GENERATOR_TYPE genType)
	{
		...
		{
		case WIND:
			thisAgent = new WindGeneratorProsumer(this.cascadeMainContext, nilDemand, capacity);
			break;
		}
	...
	public WindGeneratorProsumer createWindGenerator(double capacity, Consts.GENERATOR_TYPE genType, int nTurbines)
	{
		...
		{
		case WIND:
			thisAgent = new WindGeneratorProsumer(this.cascadeMainContext, nilDemand, capacity, nTurbines);
			break;
		}
	...
	/*
	 * 
	 * public ArrayList<ProsumerAgent> createDEFRAHouseholds(int number, String
	 * categoryFile, String profileFile) { ArrayList<ProsumerAgent> returnList =
	 * new ArrayList();
	 * 
	 * CSVReader defraCategories = null; CSVReader defraProfiles = null;
	 * 
	 * try { defraCategories = new CSVReader(categoryFile); }
	 * catch(FileNotFoundException e) { System.err.println(
	 * "ProsumerFactory: File containing DEFRA types not found at "
	 * +categoryFile); System.err.println(
	 * "ProsumerFactory: Doesn't look like this will work, terminating");
	 * System.err.println(e.getMessage());
	 * System.exit(Consts.BAD_FILE_ERR_CODE); }
	 * 
	 * try { defraProfiles = new CSVReader(profileFile); }
	 * catch(FileNotFoundException e) { System.err.println(
	 * "ProsumerFactory: File containing average profiles for DEFRA types not found at "
	 * +profileFile); System.err.println(
	 * "ProsumerFactory: Doesn't look like this will work, terminating");
	 * System.err.println(e.getMessage());
	 * System.exit(Consts.BAD_FILE_ERR_CODE); }
	 * 
	 * defraCategories.parseByColumn(); defraProfiles.parseByColumn();
	 * 
	 * //Need to think about defining the column names as consts, or otherwise
	 * working out //How we import files, whether column names are pre-ordained,
	 * or arbitrary etc.
	 * 
	 * Empirical myDist =
	 * RandomHelper.createEmpirical(ArrayUtils.convertStringArrayToDoubleArray
	 * (defraCategories.getColumn("Population_fraction")),
	 * Empirical.NO_INTERPOLATION);
	 * 
	 * for (int j = 0; j < number; j++) {
	 * 
	 * int custSegment = 0; double choiceVar = RandomHelper.nextDouble(); int i
	 ...
	 * (defraProfiles.getColumn("demand" + (custSegment - 1))), true);
	 * 
	 * prAgent.defraCategory =
	 * Integer.parseInt(defraCategories.getColumn("DEFRA_category")[custSegment
	 * - 1]); prAgent.microgenPropensity =
	 * Double.parseDouble(defraCategories.getColumn
	 * ("Microgen_propensity")[custSegment - 1]); prAgent.insulationPropensity =
	 * Double
	 * .parseDouble(defraCategories.getColumn("Insulation_propensity")[custSegment
	 * - 1]); prAgent.HEMSPropensity =
	 * Double.parseDouble(defraCategories.getColumn
	 * ("HEMS_propensity")[custSegment - 1]); prAgent.EVPropensity =
	 * Double.parseDouble(defraCategories.getColumn("EV_propensity")[custSegment
	 * - 1]); prAgent.habit =
	 * Double.parseDouble(defraCategories.getColumn("Habit_factor")[custSegment
	 * - 1]);
	 * 
	 * prAgent.hasSmartControl = true; //(RandomHelper.nextDouble() <
	 * prAgent.HEMSPropensity); prAgent.hasElectricVehicle =
	 * (RandomHelper.nextDouble() < prAgent.EVPropensity);
	 * 
