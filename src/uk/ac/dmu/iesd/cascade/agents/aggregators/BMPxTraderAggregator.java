...
public abstract class BMPxTraderAggregator extends AggregatorAgent implements IBMTrader, IPxTrader {
	...
	protected double [] arr_baselineProfile; // contains realistic load based on real data from which PN are calculated
	...
	private double[] initializePN(double[] baselineProfile) {
		...
		/******
		 * The change here, from initialising the PN from the baseline to initialising the PN stochastically makes the difference
		 * between stable market and collapse JRS.
		 */
		switch (this.type) {
		case GEN_COAL: 
