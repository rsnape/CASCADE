...
/**
 * 
 * The <em>BOD</em> class encapsulates a data structure which defines Bid and
 * Offer Data used in the UK balancing mechanism electricity market.
 * 
...
public class BOD
{
	...
	public String toString()
	{
		...
		String toString = "BOD: ownerID: " + this.ownerID + ", isAccepted: " + this.isAccepted + ", pairID: " + this.pairID + ", sp: "
				+ this.sp + ", level: " + this.level + ", submittedBO (price): " + this.submittedBO + ", probArray: "
				+ Arrays.toString(this.arr_probabilities);

		return toString;
	}

	/*
	 * public Double getOfferInDouble() { return new Double(offer); }
	 */
	...
	public BOD(int oID, int settlPeriod, int pID, double levelAmount, double[] bidOrOfferArray, double[] propensityArray)
	{
		...
		this.arr_BO = new double[ASTEMConsts.BMU_BO_NUM_OF_CHOICE];
		this.arr_propensities = new double[ASTEMConsts.BMU_BO_NUM_OF_CHOICE];
		this.arr_probabilities = new double[ASTEMConsts.BMU_BO_NUM_OF_CHOICE];

		System.arraycopy(bidOrOfferArray, 0, this.arr_BO, 0, bidOrOfferArray.length);
		System.arraycopy(propensityArray, 0, this.arr_propensities, 0, propensityArray.length);

		double sumOfPropensities = ArrayUtils.sum(this.arr_propensities);

		for (int i = 0; i < this.arr_propensities.length; i++)
		{
			this.arr_probabilities[i] = this.arr_propensities[i] / sumOfPropensities;
		}

		int ind = RandomHelper.getUniform().nextIntFromTo(0, 2); // TODO: JRS :
																	// Why pick
																	// randomly
																	// from
																	// first 3,
