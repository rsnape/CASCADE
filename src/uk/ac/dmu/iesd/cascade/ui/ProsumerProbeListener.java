...
/***
 * Class to listen for probe of prosumer and produce custom information panel
 * when triggered.
 * 
...
public class ProsumerProbeListener implements ProbeListener
{
	...
	static class ProbeUpdater implements IAction
	{
		...
		public void execute()
		{
			this.probeListener.scheduledUpdate();
		}
	}

	/**
	 * Modification: modified and added new probing elements such as pie and
	 * line charts for monitoring the electricity consumption (Babak Mahdavi)
	 */
	@Override
	public void objectProbed(ProbeEvent evt)
	{
		...
		for (Object thisObj : probedObjects)
		{
			if (thisObj instanceof HouseholdProsumer)
			{
				...
				this.probedAgents.add(thisAgent);
				...
				agentProbeFrame.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
				...
				// variablesBox.setLayout(new BoxLayout(variablesBox,
				// BoxLayout.PAGE_AXIS));
				...
				{
					DefaultCategoryDataset tempDataset = this.createTemperatureDataset(thisAgent);
					// based on the dataset we create the chart
					JFreeChart tempChart = this.createLineChart(tempDataset, "Previous day temperatures", "Half hour", "degrees C");
				...
				CategoryTableXYDataset dataset = this.createDataset(thisAgent);
				// based on the dataset we create the chart
				final JFreeChart chart = this.createChart(dataset, "Previous day demand by type");
				...
				saveAsButton.addMouseListener(new MouseAdapter()
				{
					...
					{
						String name = thisAgent.getAgentName() + "_demand_at_tick_" + ProsumerProbeListener.this.mainContext.getTickCount()
								+ ".png";
				...
				DefaultPieDataset pieDataset = this.createPieDataset(thisAgent);
				JFreeChart pieChart = this.createPieChart(pieDataset, "Demands Proportion");
				...
				loadsProfilesLineChartButton.addMouseListener(new MouseAdapter()
				{
					...
					public void mouseClicked(MouseEvent e)
					{
						ProsumerProbeListener.this.createDataset(thisAgent);
					}
				});
				...
				allBarChartButton.addMouseListener(new MouseAdapter()
				{
					...
					public void mouseClicked(MouseEvent e)
					{
						IndexedIterable<HouseholdProsumer> iIterOfHouseholdProsumers = ProsumerProbeListener.this.mainContext
								.getObjects(HouseholdProsumer.class);
						ArrayList<HouseholdProsumer> list_hhProsumers = IterableUtils.Iterable2ArrayList(iIterOfHouseholdProsumers);
						CategoryTableXYDataset barDatasetOfAllHHProsConsumption = ProsumerProbeListener.this
								.createStackedBarDataset4DemandOfAllHHPros(list_hhProsumers);
						int now = ProsumerProbeListener.this.mainContext.getTickCount();
						int start = now - (now % ProsumerProbeListener.this.mainContext.ticksPerDay);
						final JFreeChart barChart4AllHHProsConsumption = ProsumerProbeListener.this
								.createChart(barDatasetOfAllHHProsConsumption, "Total Demands Proportion of All HHPros (from " + start
										+ " to " + (start + ProsumerProbeListener.this.mainContext.ticksPerDay) + " evaluated @t=" + now
										+ ")");
						TickUnits fourTicks = new TickUnits();
						fourTicks.add(new NumberTickUnit(4));
						...
						saveAsButton.addMouseListener(new MouseAdapter()
						{
							...
							public void mouseClicked(MouseEvent e)
							{
								String name = "all_households_demand_at_tick_" + ProsumerProbeListener.this.mainContext.getTickCount()
										+ ".png";
	...
	/**
	 * Creates and returns a category dataset of the demand of a given
	 * HHProsumer to be used in a line chart.
	 * 
	...
	/**
	 * Creates and returns a pie chart dataset using the demand of a given
	 * HHProsumer
	 * 
	 * @author Babak Mahdavi
	 * @param hhAgent
	...
	/**
	 * Creates and returns a pie chart dataset using the demand of all
	 * HHProsumers
	 * 
	 * @author Babak Mahdavi
	...
	/**
	 * Creates and returns a pie chart dataset using the average demand of all
	 * HHProsumers
	 * 
	 * @author Babak Mahdavi
	 * @param list_hhProsumers
	...
	/**
	 * Creates and returns a line chart (of JFreeChart type) using a given
	 * category dataset, title and y-axis label.
	 * 
	 * @author Babak Mahdavi
	 * @param dataset
	 * @param title
	...
	// @ScheduledMethod(start = 0, interval = 48, shuffle = true, priority =
	// Consts.PROBE_PRIORITY)
	public void scheduledUpdate()
	{
		// Bit of a hack based on the thought that the ArrayLists will both be
		// in the same order
		// Could be we should have a Collection of Pairs to
		// Make this more explicit.
		int i = 0;
		for (HouseholdProsumer hhProbedAgent : this.probedAgents)
		{
			if (hhProbedAgent.isHasElectricalSpaceHeat())
			{
				this.charts.get(i).getCategoryPlot().setDataset(this.createTemperatureDataset(hhProbedAgent));
				// charts.get(i+1).getCategoryPlot().setDataset(createDataset(hhProbedAgent));

				this.charts.get(i + 1).getXYPlot().setDataset(this.createDataset(hhProbedAgent));
				((PiePlot) this.charts.get(i + 2).getPlot()).setDataset(this.createPieDataset(hhProbedAgent));
			...
			else
			{
				...
				this.charts.get(i).getXYPlot().setDataset(this.createDataset(hhProbedAgent));
				((PiePlot) this.charts.get(i + 1).getPlot()).setDataset(this.createPieDataset(hhProbedAgent));
	...
	public ProsumerProbeListener(CascadeContext context)
	{
		...
		RunEnvironment
				.getInstance()
				.getCurrentSchedule()
				.schedule(ScheduleParameters.createRepeating(RepastEssentials.GetTickCount() + 1, context.getNbOfTickPerDay(), ScheduleParameters.END), new ProbeUpdater(
						this));
