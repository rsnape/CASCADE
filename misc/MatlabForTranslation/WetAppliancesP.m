function WetrP = WetAppliancesP(S,NoPros,Pros)
%This is a wet appliance function for CASCADE prosumer experiments that
%seeks the cheapest timeslot(s) to run in.
...
%AggLkWh
%
%Now run a simple optimisation which finds the cheapest timeslot to start
%in the next Tw timeslots once a user has decided to run.
    ...
    for k=1:Occ %run appliances depending on occupancy
        ...
        while i<48
            if rand < Psw(i)
                if i<=(48-Tw)
                Smin=min(S(i:i+Tw-1));
                k=find(S(i:i+Tw-1)==Smin);    
                L(i+k-1)=W;
                L(i+k)=W;
                i=i+Tw-1;%step beyond Tw
                elseif i<48
                L(i)=W;
                L(i+1)=W;
                i=i+1;
